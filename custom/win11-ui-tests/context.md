...
- **Hive `HKLM\DEFAULT`** — po `Mount-RegistryHive` zapisuj wartości przez `Set-RegistryKeyDword -UseRegExe` (`reg.exe`), nie `New-ItemProperty`; inaczej `reg unload` kończy się „Access is denied”.
- **Sysprep** — nieskończona pętla `IMAGE_STATE_COMPLETE` oznacza że `Sysprep.exe /generalize` **padł w tle** (najczęściej AppX zainstalowany per-user bez provisioningu — błąd `0x80073cf2`). Przed generalize uruchamia się `Prepare-UiTestsSysprep.ps1` (usuwa subst D:, AppX); `Invoke-UiTestsSysprep.ps1` ma timeout 45 min i wypisuje tail `Panther\setuperr.log`. Logi diagnostyczne: `C:\Windows\System32\Sysprep\Panther\setupact.log`.
