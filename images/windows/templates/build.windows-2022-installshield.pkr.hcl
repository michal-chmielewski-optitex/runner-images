build {
  ...
  name = "windows-2022-installshield"
  ...
  # InstallShield 2025 SAB — baked on the image; license + builds via Revenera ADO extension in pipeline.
  provisioner "powershell" {
    environment_vars = [
      "IMAGE_FOLDER=${var.image_folder}",
      "TEMP_DIR=${var.temp_dir}",
      "INSTALLSHIELD_ARTIFACTS_STORAGE_ACCOUNT=${var.installshield_artifacts_storage_account}",
      "INSTALLSHIELD_INSTALLER_URL=${var.installshield_installer_url}",
      "INSTALLSHIELD_INSTALLER_PATH=${var.installshield_installer_path}"
    ]
    scripts = [
      "${path.root}/../scripts/build/Install-InstallShield.ps1"
    ]
  }
  ...
  provisioner "file" {
    destination = "${path.root}/../Windows2022-InstallShield-Readme.md"
    direction   = "download"
